...
struct EmployeeOutcomeAssignmentSheet: View {
        ...
        HStack(spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 22) {
                ...
                if let submissionError {
                    ...
                    Text("\(employee.name) needs at least one skill before assignment. Review their skills in Company.")
                        .font(.caption)
                        .foregroundStyle(EditorialOfficeTheme.graphite)
                        .fixedSize(horizontal: false, vertical: true)
                }
                ...
                HStack(alignment: .center, spacing: 14) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text(model.organization.executionMode == .demo ? "Practice with the Demo team" : "Work with Local Codex")
                            .font(.callout.weight(.medium))
                        Text("No self-granted permissions, publishing, spending, or writes outside this company folder.")
                            .font(.caption)
                            .foregroundStyle(EditorialOfficeTheme.graphite)
                    }
                    Spacer(minLength: 12)
    ...
    private var identityPanel: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Text("These are instructions, not permissions. \(employee.name) can ask for access but cannot grant it.")
                .font(.caption)
    ...
    private func submit() {
        submissionError = nil
        guard model.submitEmployeeOutcome(employeeID: employee.id, outcome: outcome, context: context) else {
