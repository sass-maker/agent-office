...
struct OfficeSceneView: NSViewRepresentable {
    ...
    func updateNSView(_ view: SKView, context: Context) {
        ...
        let hasActiveMovement = organization.workdayStatus == .active
            || organization.employees.contains { ![.resting, .waiting, .celebrating].contains($0.status) }
        view.preferredFramesPerSecond = reduceMotion && !hasActiveMovement
            ? 1
...
final class OfficeScene: SKScene {
    ...
    override func didChangeSize(_ oldSize: CGSize) {
        ...
        if oldSize.width > 0, oldSize.height > 0 {
            ...
            employeeNodes.values.forEach { node in
    ...
    func apply(organization: OrganizationState, selectedEmployeeID: String?, showsLabels: Bool, reduceMotion: Bool) {
    ...
    private func render(organization: OrganizationState, selectedEmployeeID: String?, showsLabels: Bool, reduceMotion: Bool) {
        guard size.width > 0, size.height > 0 else { return }
        loadBackground()
        ...
        for (index, employee) in officeEmployees.enumerated() {
            ...
            let signature = "\(station.rawValue):\(slot):\(employee.status.rawValue):\(currentTask?.status.rawValue ?? "none")"
    ...
    private func loadBackground() {
        ...
        if let url = Bundle.module.url(forResource: "editorial-office-background", withExtension: "png"),
...
final class EmployeeSpriteNode: SKNode {
    ...
    init(employee: Employee) {
        ...
        character.shader = SKShader(source: """
            void main() {
    ...
    func update(employee: Employee, task: WorkTask?) {
        ...
        let color: NSColor = switch employee.status {
    ...
    func travel(along route: [CGPoint], in sceneSize: CGSize, idleRoute: [CGPoint]?, speed: CGFloat) {
        ...
        if let idleRoute, idleRoute.count > 1 {
            ...
            for point in idleRoute.dropFirst() + [idleRoute[0]] {
                ...
                idleActions.append(.run { [weak self] in self?.beginWalking(horizontalDirection: direction) })
                let move = SKAction.move(to: point, duration: duration)
                move.timingMode = .easeInEaseOut
    ...
    private func beginWalking(horizontalDirection: CGFloat) {
        ...
        if !walkTextures.isEmpty {
            character.run(
                .repeatForever(.animate(with: walkTextures, timePerFrame: 0.13, resize: false, restore: false)),
                withKey: "walk-frames"
            )
        }
