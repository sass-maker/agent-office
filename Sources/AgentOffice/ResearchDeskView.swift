...
struct ResearchAssignmentSheet: View {
        ...
        VStack(alignment: .leading, spacing: 22) {
            HStack(alignment: .top, spacing: 18) {
                ...
                VStack(alignment: .leading, spacing: 5) {
                    ...
                    Text("Mira will frame the request, Nia will research it, and the result stays in this company folder.")
                        .font(.callout)
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                TextField(
                    "",
                    text: $context,
                    prompt: Text("Audience, constraints, products to include, or the decision this should support")
                        .foregroundStyle(ink.opacity(0.52)),
                    axis: .vertical
                )
                    .textFieldStyle(.plain)
    ...
    private var modeTitle: String {
        ...
        return model.webResearchGranted ? "Read-only web research granted" : "Nia will ask before researching"
    }

    private var modeDetail: String {
        if model.organization.executionMode == .demo {
            return "No web research will occur. The brief will be clearly marked as a rehearsal based only on your context."
        }
        if model.webResearchGranted {
            return "Nia may read public web sources. She cannot publish, message people, or change external systems."
        }
