...
struct OnboardingView: View {
  ...
  @State private var selectedStarterPackageIDs = Set([
    "starter.mira", "starter.maya", "starter.nia", "starter.theo", "starter.iris",
  ])
        ...
        Button {
          ...
          HStack(alignment: .top, spacing: 10) {
            VStack(spacing: 0) {
              ZStack {
                ...
                Text("\(index + 1)")
                  .font(.caption)
                  .foregroundStyle(
                    index == step ? EditorialOfficeTheme.onInk : EditorialOfficeTheme.ink)
              }
            ...
            if !compact {
              Text(title)
                .font(
                  .system(.callout, design: .default, weight: index == step ? .medium : .regular)
                )
                .foregroundStyle(
                  index <= step ? EditorialOfficeTheme.ink : EditorialOfficeTheme.graphite
                )
                .padding(.top, 5)
                .lineLimit(1)
  ...
  private var officePreview: some View {
    GeometryReader { proxy in
      ZStack {
        ...
        if let assistant = model.organization.employee("mira")
          ?? model.organization.employees.first(where: { $0.kind == .ai })
        {
          VStack {
            Spacer()
  ...
  private static let officePreviewImage: NSImage? = {
    guard
      let url = Bundle.module.url(forResource: "editorial-office-background", withExtension: "png")
    else {
      return nil
    }
  ...
  private var welcome: some View {
    VStack(alignment: .leading, spacing: 28) {
      Text(
        "Set one outcome. Your employees will decide what comes next, work through it together, and bring you in where judgment is needed."
      )
      ...
      VStack(alignment: .leading, spacing: 18) {
        promise(
          "Named employees",
          detail:
            "Each person has a role, responsibilities, skills, memory, and a place in the company.")
        promise(
          "A living workplace",
          detail:
            "Their real tasks, handoffs, blockers, and artifacts are visible throughout the day.")
        promise(
          "Local by default",
          detail: "Your organization and its work stay in a folder you control on this Mac.")
      }
    }
  }
  ...
  private var organizationSetup: some View {
    VStack(alignment: .leading, spacing: 28) {
      ...
      editorialLineField(
        "Your name", prompt: "What should your assistant call you?", text: $ownerName)
      editorialTextField(
  ...
  private var productSetup: some View {
    VStack(alignment: .leading, spacing: 32) {
      ...
      editorialLineField(
        "Current stage", prompt: "Proof of concept, early product, growing…", text: $stage)
    }
  }
  ...
  private var readyToOpen: some View {
    VStack(alignment: .leading, spacing: 24) {
      VStack(alignment: .leading, spacing: 7) {
        Text(
          "Review the people you want to hire. Each candidate arrives with declared skills and boundaries; connections are never granted by hiring."
        )
        .font(.system(.title3, design: .serif))
        .lineSpacing(4)
        Text(
          "Open Contract to inspect execution, review, connections, package source, and reduced-mode behavior."
        )
        .font(.caption)
        .foregroundStyle(EditorialOfficeTheme.graphite)
      }
      ...
      if model.codexAvailable {
        ...
        editorialToggle(
          ...
          detail:
            "It is temporarily unavailable. This choice will be preserved and employees will wait for it to return.",
          isOn: $useLocalCodex
        )
        .disabled(true)
      }
      ...
      if selectedStarterPackageIDs.isEmpty {
        Label(
          "Select at least one employee to open the office.", systemImage: "exclamationmark.circle"
        )
  ...
  private func candidateFolio(_ package: EmployeePackage) -> some View {
    ...
    return VStack(alignment: .leading, spacing: 0) {
      HStack(alignment: .top, spacing: 12) {
        Toggle(
          ...
          isOn: Binding(
            ...
            set: { isSelected in
              if isSelected {
                selectedStarterPackageIDs.insert(package.id)
              } else {
                selectedStarterPackageIDs.remove(package.id)
              }
            }
          )
        )
        ...
        .accessibilityHint(
          "Hiring creates an employee and local working contract, but grants no external authority")
        ...
        VStack(alignment: .leading, spacing: 5) {
          ...
          Label("Local sandbox · reviewed plans · no publishing", systemImage: "lock")
            ...
            .lineLimit(1)
            .help(
              "They work only in the selected company folder. You review plans before execution, and hiring grants no publishing authority."
            )
        }
        ...
        .accessibilityLabel(
          expanded
            ? "Hide \(package.name) contract details" : "Show \(package.name) contract details"
        )
      ...
      if expanded {
        expandedContractDetails(for: package)
      }
    }
  ...
  private func expandedContractDetails(for package: EmployeePackage) -> some View {
    VStack(alignment: .leading, spacing: 10) {
      Rectangle()
      ...
      candidateContractFact(
        "Included skills", package.skills.map(\.name).joined(separator: ", "))
      candidateContractFact("Connections", candidateConnectionNames(package))
      candidateContractFact(
        "Execution",
        package.preferredProvider == .localCodex
          ? "Local sandbox · Local Codex preferred" : "Local sandbox · practice mode ready")
      candidateContractFact(
  ...
  private var finalHireSummary: String {
    ...
    let connections =
      allowWebResearch && useLocalCodex ? "Nia gets read-only research" : "no connections granted"
    let openingWork = startImmediately ? "first outcomes prepared" : "no work starts yet"
    return
      "\(selectedStarterPackageIDs.count) \(noun) · \(execution) · \(connections) · no publishing · \(openingWork)"
  }
  ...
  private func editorialLineField(_ label: String, prompt: String, text: Binding<String>)
    -> some View
  {
    VStack(alignment: .leading, spacing: 10) {
      Text(label)
  ...
  private var currentStepIsValid: Bool {
    switch step {
    case 1:
      !organizationName.trimmed.isEmpty && !ownerName.trimmed.isEmpty && !purpose.trimmed.isEmpty
