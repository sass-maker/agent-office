...
struct MissionView: View {
  ...
  var body: some View {
    ...
    .onAppear {
      ...
      selectedOutcomeID =
        model.organization.managementInbox.first?.outcomeID
        ?? model.organization.employeeOutcomes.first?.id
    }
    ...
    .onChange(of: missionDraft) { _, value in
      onDirtyChange(
        value.trimmingCharacters(in: .whitespacesAndNewlines) != model.organization.outcome)
    }
  ...
  private func outcomeLedger(compact: Bool) -> some View {
    ...
    return VStack(alignment: .leading, spacing: 0) {
      HStack {
        ...
        Text("\(outcomes.filter { !$0.status.isTerminal }.count) open").font(.caption)
          .foregroundStyle(EditorialOfficeTheme.graphite)
        Spacer()
        Text(
          "\(model.runningEmployeeIDs.count)/\(model.organization.effectiveConcurrencyLimit) running"
        )
      ...
      if outcomes.isEmpty {
        HStack {
          Text("Give a hired employee an outcome to create the first commitment.").font(.callout)
            .foregroundStyle(EditorialOfficeTheme.graphite)
          Spacer()
          Button("Open office", action: onOpenOffice).buttonStyle(.plain).underline()
        }
  ...
  private func outcomeRow(_ outcome: EmployeeOutcome, compact: Bool) -> some View {
    ...
    return VStack(alignment: .leading, spacing: 0) {
      Button {
        ...
        HStack(spacing: 12) {
          if let employee {
            EmployeePortrait(employee: employee, size: CGSize(width: 30, height: 36)).saturation(0)
          }
          VStack(alignment: .leading, spacing: 2) {
            Text(outcome.outcome).font(.callout.weight(.semibold)).lineLimit(compact ? 2 : 1)
            Text(
              "\(employee?.name ?? outcome.assigneeID) · \(outcomeStatusLabel(outcome)) · \(outcome.effectivePriority.rawValue) priority"
            )
            .font(.caption).foregroundStyle(EditorialOfficeTheme.graphite)
          }
          Spacer()
          if model.runningEmployeeIDs.contains(outcome.assigneeID) {
            ProgressView().controlSize(.small).accessibilityLabel("Running")
          }
          Text(
            "\(outcome.taskIDs.filter { model.organization.task($0)?.status == .done }.count)/\(outcome.taskIDs.count)"
          )
          .font(.caption.monospacedDigit()).foregroundStyle(EditorialOfficeTheme.graphite)
      ...
      .buttonStyle(.plain)
      .accessibilityLabel(
        "\(outcome.outcome), owned by \(employee?.name ?? outcome.assigneeID), \(outcomeStatusLabel(outcome))"
      )

      if selected {
        outcomeManagement(outcome).padding(.horizontal, compact ? 24 : 38).padding(.bottom, 12)
      }
    }
    .overlay(alignment: .bottom) {
      Rectangle().fill(EditorialOfficeTheme.rule.opacity(0.45)).frame(height: 1)
    }
  }
    ...
    VStack(alignment: .leading, spacing: 9) {
      if !outcome.effectiveAcceptanceCriteria.isEmpty {
        Text("Acceptance · \(outcome.effectiveAcceptanceCriteria.joined(separator: " · "))").font(
          .caption
        ).foregroundStyle(EditorialOfficeTheme.ink.opacity(0.72))
      }
      ...
      HStack(spacing: 8) {
        ...
        case .proposed:
          Button("Approve plan") { model.approveEmployeeOutcomePlan(outcome.id) }.buttonStyle(
            EditorialPrimaryButtonStyle())
          Button("Return plan") {
            model.returnEmployeeOutcomePlan(outcome.id, instruction: managementNote)
          }.buttonStyle(EditorialSecondaryButtonStyle()).disabled(
            managementNote.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
        case .waiting, .failed:
          Button("Send reply") { model.replyToEmployeeOutcome(outcome.id, message: managementNote) }
            .buttonStyle(EditorialPrimaryButtonStyle()).disabled(
              managementNote.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
          Button("Stop") { pendingStopOutcomeID = outcome.id }.buttonStyle(
            EditorialSecondaryButtonStyle())
        case .delivered:
          Button("Accept") { model.acceptEmployeeOutcome(outcome.id, note: managementNote) }
            .buttonStyle(EditorialPrimaryButtonStyle())
          Button("Request changes") {
            model.requestEmployeeOutcomeRevision(outcome.id, feedback: managementNote)
          }.buttonStyle(EditorialSecondaryButtonStyle()).disabled(
            managementNote.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
        case .queued, .approved:
          Button("Move up") { model.reorderEmployeeOutcome(outcome.id, offset: -1) }.buttonStyle(
            EditorialSecondaryButtonStyle())
          Button("Move down") { model.reorderEmployeeOutcome(outcome.id, offset: 1) }.buttonStyle(
            EditorialSecondaryButtonStyle())
          Button("Stop") { pendingStopOutcomeID = outcome.id }.buttonStyle(.plain)
        case .planning, .working, .revision:
          Button("Stop outcome") { pendingStopOutcomeID = outcome.id }.buttonStyle(
            EditorialSecondaryButtonStyle())
    ...
    HStack(alignment: .center, spacing: 28) {
      VStack(alignment: .leading, spacing: 7) {
        ...
        TextField("What should the organization make true?", text: $missionDraft, axis: .vertical)
          ...
          .font(
            .system(
              size: compact ? missionTitleSize * 0.84 : missionTitleSize, weight: .regular,
              design: .serif)
          )
          .tracking(-0.5)
        ...
        HStack(spacing: 7) {
          Label(
            model.organization.workdayStatus == .complete
              ? "Mission delivered" : "\(openTaskCount) open",
            systemImage: model.organization.workdayStatus == .complete
          ...
          if missionIsDirty {
            ...
            Button(isSavingMission ? "Saving…" : "Save mission", action: saveMission)
              ...
              .disabled(
                isSavingMission
                  || missionDraft.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
          }
        }
        .font(.callout)
      ...
      if !compact {
        VStack(alignment: .trailing, spacing: 8) {
          Text(
            managementDecisionCount == 0
              ? "Nothing needs you" : "\(managementDecisionCount) owner decisions"
          )
          .font(.caption.weight(.medium))
          .foregroundStyle(
            managementDecisionCount == 0
              ? EditorialOfficeTheme.graphite : EditorialOfficeTheme.attention)
  ...
  private func employeeTaskGroup(_ employee: Employee, tasks: [WorkTask], compact: Bool)
    -> some View
  {
    VStack(alignment: .leading, spacing: 0) {
      HStack(spacing: 9) {
  ...
  private func taskRow(_ task: WorkTask, compact: Bool) -> some View {
    ...
    .buttonStyle(.plain)
    .accessibilityLabel(
      "\(task.title). \(task.detail). \(statusLabel(task.status)), assigned to \(model.employeeName(task.assigneeID))"
    )
      ...
      VStack(alignment: .leading, spacing: 0) {
        HStack {
          ...
          Menu {
            ...
            if task.status != .done {
              Divider()
              ForEach(model.organization.activeAIEmployees.filter { $0.id != task.assigneeID }) {
                employee in
                Button("Reassign to \(employee.name)", systemImage: "person.2") {
                  model.reassignEmployeeTicket(
                    task.id, employeeID: employee.id, reason: "Owner reassigned from Mission.")
                }
              }
            }
          } label: {
        ...
        if task.effectiveAccountableEmployeeID != task.assigneeID {
          Text(
            "Delegated by \(model.employeeName(task.effectiveAccountableEmployeeID)) · \(task.delegationReason ?? "bounded collaboration")"
          )
              ...
              HStack(alignment: .top, spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                  HStack {
                    ...
                    Spacer()
                    Text(
                      item.createdAt, format: .dateTime.month(.abbreviated).day().hour().minute()
                    )
                    .font(.caption2)
                    .foregroundStyle(EditorialOfficeTheme.graphite)
                  }
  ...
  private func inspectorRow<Content: View>(_ title: String, @ViewBuilder content: () -> Content)
    -> some View
  {
  ...
  private var missionTasks: [WorkTask] {
    ...
    return outcomeTaskIDs.isEmpty
      ? model.organization.tasks
      : model.organization.tasks.filter { outcomeTaskIDs.contains($0.id) }
  }
