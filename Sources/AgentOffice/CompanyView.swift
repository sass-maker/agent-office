...
struct CompanyView: View {
        ...
        .onAppear {
            ...
            if let initialEmployeeID,
               model.organization.employee(initialEmployeeID) != nil {
                ...
                selectedEmployeeID = model.organization.employee("maya")?.id
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(spacing: 0) {
                ForEach(CompanySection.allCases) { item in
                    ...
                    .help(profileIsDirty && section == .overview && item != .overview
    ...
    private func memberFolio(
        ...
        return Button {
            ...
            .overlay {
                Rectangle()
                    .stroke(selected ? EditorialOfficeTheme.ink : EditorialOfficeTheme.rule, lineWidth: selected ? 1.5 : 1)
            }
            .shadow(color: EditorialOfficeTheme.sidebarInk.opacity(selected ? 0.2 : 0.12), radius: 8, x: 3, y: 6)
        }
        .buttonStyle(.plain)
        ...
        ScrollView {
            if let employee = selectedEmployee {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    summarySection("Reports", value: reports(for: employee).map(\.name).joined(separator: "\n"))
                    summarySection("Collaborates with", value: collaborators(for: employee).map(\.name).joined(separator: "\n"))
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 28) {
                ...
                VStack(alignment: .leading, spacing: 22) {
                    companyTextArea("Operating principles", prompt: "How should people make decisions and work together?", text: $profileDraft.operatingPrinciples)
                    companyTextArea("Constraints and safe claims", prompt: "What must the team never assume or do?", text: $profileDraft.constraints)
                }
            }
    ...
    private var identityFields: some View {
        VStack(alignment: .leading, spacing: 22) {
            ...
            companyTextArea("Purpose", prompt: "Why does this organization exist?", text: $profileDraft.purpose)
            companyTextArea("Current mission", prompt: "What must the organization make true now?", text: $outcomeDraft)
        }
    ...
    private var productFields: some View {
        VStack(alignment: .leading, spacing: 22) {
            Text("Product context")
                .font(.system(.title3, design: .serif, weight: .medium))
            companyTextArea("Product", prompt: "What are you building or offering?", text: $profileDraft.product)
            companyTextArea("Audience", prompt: "Who is it for?", text: $profileDraft.audience)
            companyField("Current stage", prompt: "Proof of concept, early product, growing…", text: $profileDraft.stage)
        }
        .frame(maxWidth: .infinity, alignment: .topLeading)
    }
    ...
    private func employeeIdentity(_ employee: Employee) -> some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            identityFact("Member kind", value: employee.kind == .human ? "Human" : "AI employee", icon: "person")
            identityFact("Reports to", value: managerName(for: employee), icon: "person.line.dotted.person")
            identityFact("Direct reports", value: reports(for: employee).map(\.name).joined(separator: ", "), icon: "person.2")
                ...
                HStack(spacing: 0) {
                    ForEach(EmployeeDetailTab.allCases) { tab in
                        Button {
                            ...
                            Text(tab.rawValue)
                                .font(.system(.title3, design: .serif, weight: employeeTab == tab ? .medium : .regular))
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(spacing: 0) {
                ...
                if activeTasks(for: employee).isEmpty {
                    Text("No active work. The employee will pick up from the task board when the next day begins.")
                        .font(.callout)
                        .foregroundStyle(EditorialOfficeTheme.graphite)
                        .padding(.vertical, 18)
                }
            ...
            HStack(alignment: .top, spacing: 28) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    relationshipRow("Direct reports", value: reports(for: employee).map(\.name).joined(separator: ", "))
                    relationshipRow("Collaborators", value: collaborators(for: employee).map(\.name).joined(separator: ", "))
                }
                .frame(maxWidth: .infinity, alignment: .leading)
            }
    ...
    private func employeeSkillDetail(_ employee: Employee) -> some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            ForEach(skills(for: employee)) { skill in
                ...
                .padding(.vertical, 16)
                .overlay(alignment: .bottom) { Rectangle().fill(EditorialOfficeTheme.rule.opacity(0.6)).frame(height: 1) }
            }
            ...
            VStack(alignment: .leading, spacing: 22) {
                ledgerSection("Skills") {
                    ForEach(skills(for: employee)) { skill in
                        ...
                        .font(.callout)
                        .padding(.vertical, 7)
                        .overlay(alignment: .bottom) { Rectangle().fill(EditorialOfficeTheme.rule.opacity(0.55)).frame(height: 1) }
                    }
                }
                ...
                ledgerSection("Blockers") {
                    ...
                    Text(items.isEmpty ? "None" : items.map(\.title).joined(separator: "\n"))
                        .font(.callout)
                        .foregroundStyle(items.isEmpty ? EditorialOfficeTheme.ink : EditorialOfficeTheme.attention)
                }
            }
            .padding(24)
        }
                ...
                if page == .skills {
                    ForEach(model.organization.knowledge?.skillDefinitions ?? []) { skill in
                        catalogueRow(
                            ...
                            trailing: model.organization.employeesWithSkill(skill.id).map(\.name).joined(separator: ", ")
                        )
                    }
                } else {
                    ForEach(model.organization.knowledge?.connectionDefinitions ?? []) { connection in
                        let granted = connection.capabilityID.map { capability in
                            model.organization.employees.filter { $0.capabilityGrants.contains(capability) }.map(\.name)
                        } ?? []
                        catalogueRow(
    ...
    private func catalogueRow(title: String, subtitle: String, detail: String, trailing: String) -> some View {
        HStack(alignment: .top, spacing: 20) {
            ...
            Text(trailing.isEmpty ? "Coverage gap" : trailing)
                ...
                .foregroundStyle(trailing.isEmpty ? EditorialOfficeTheme.attention : EditorialOfficeTheme.ink)
                .frame(width: 170, alignment: .leading)
        }
        .padding(.vertical, 17)
        .overlay(alignment: .bottom) { Rectangle().fill(EditorialOfficeTheme.rule.opacity(0.72)).frame(height: 1) }
    }
    ...
    private func identityFact(_ title: String, value: String, icon: String) -> some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text(value.isEmpty ? "None" : value).font(.callout).foregroundStyle(EditorialOfficeTheme.graphite)
            }
        }
        .padding(.bottom, 15)
    }
    ...
    private func employeeTaskRowContent(_ task: WorkTask, showsArtifact: Bool) -> some View {
        HStack(spacing: 14) {
            Image(systemName: task.status == .done ? "checkmark" : "arrow.up")
                .foregroundStyle(task.status == .done ? EditorialOfficeTheme.success : EditorialOfficeTheme.attention)
                .frame(width: 18)
            ...
            Text(task.updatedAt, format: .dateTime.month(.abbreviated).day()).font(.caption).frame(width: 54)
            if showsArtifact {
    ...
    private func relationshipRow(_ title: String, value: String) -> some View {
        HStack(alignment: .firstTextBaseline, spacing: 10) {
            ...
            Text(value.isEmpty ? "None" : value).font(.caption).foregroundStyle(EditorialOfficeTheme.ink.opacity(0.76))
        }
    }

    private func ledgerSection<Content: View>(_ title: String, @ViewBuilder content: () -> Content) -> some View {
        VStack(alignment: .leading, spacing: 9) {
            Text(title).font(.headline)
            content()
        }
        .padding(.bottom, 18)
        .overlay(alignment: .bottom) { Rectangle().fill(EditorialOfficeTheme.rule.opacity(0.7)).frame(height: 1) }
    }
    ...
    private var companyPurpose: String {
        ...
        return value.isEmpty ? "A company where people and AI employees make useful work together." : value
    }
    ...
    private func collaborators(for employee: Employee) -> [Employee] {
        ...
        let IDs = Set(model.organization.tasks.filter {
            !$0.dependencyIDs.filter(taskIDs.contains).isEmpty || taskIDs.contains($0.id)
        }.flatMap { [$0.assigneeID, $0.reviewerID].compactMap { $0 } })
        return model.organization.employees.filter { IDs.contains($0.id) && $0.id != employee.id }
    }
    ...
    private var currentSnapshot: CompanyDraftSnapshot {
        CompanyDraftSnapshot(name: nameDraft, owner: ownerDraft, outcome: outcomeDraft, profile: profileDraft)
    }
    ...
    private func conciseProduct(from value: String) -> String {
        ...
        guard let heading = lines.firstIndex(where: { $0.trimmingCharacters(in: .whitespaces) == "## Product" }) else { return value }
