...
public enum ExecutiveAssistant {
  public static func morningBrief(for state: OrganizationState, humanID: String = "owner")
    -> AssistantBrief?
  {
    ...
    if !state.hasMeaningfulProductBrief {
      decisions.append(
        "Tell the team what the product is, who it serves, and which claims it may safely make.")
    }
    ...
    case .active:
      summary =
        "The team is carrying \(total - completed) open task\(total - completed == 1 ? "" : "s") toward \(state.outcome)"
    case .complete:
      summary =
        "The team completed the bounded workday and left \(state.artifacts.count) inspectable artifact\(state.artifacts.count == 1 ? "" : "s")."
    case .ending, .resting:
      summary =
        completed == 0
    ...
    if !decisions.isEmpty {
      ...
      nextAction =
        "Next: \(nextTask.title), owned by \(state.employee(nextTask.assigneeID)?.name ?? nextTask.assigneeID)."
