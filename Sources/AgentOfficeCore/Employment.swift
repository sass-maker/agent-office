...
public struct ContractChange: Identifiable, Codable, Sendable, Equatable {
    ...
    public init(id: String = UUID().uuidString, employeeID: String, field: String, previousValue: String, newValue: String, actorID: String, reason: String, createdAt: Date) {
        self.id = id
        self.employeeID = employeeID
        self.field = field
...
public struct SupervisionEvent: Identifiable, Codable, Sendable, Equatable {
    ...
    public init(id: String = UUID().uuidString, kind: SupervisionEventKind, actorID: String, employeeID: String, outcomeID: String? = nil, taskID: String? = nil, message: String, priorValue: String? = nil, createdAt: Date) {
        self.id = id
        self.kind = kind
        self.actorID = actorID
        self.employeeID = employeeID
...
public enum EmployeePackageError: LocalizedError, Equatable {
    ...
    public var errorDescription: String? {
        ...
        case .invalidIdentifier: "Use a stable lowercase package identifier containing letters, numbers, dots, or hyphens."
        ...
        case .unsupportedProvider: "The preferred execution provider is not included in the package's supported providers."
        case .unsafeSecret(let key): "Employee packages cannot contain credentials or secret-shaped data (\(key))."
        case .executableReference(let value): "Employee packages are declarative and cannot reference executable code (\(value))."
...
public enum EmploymentError: LocalizedError, Equatable {
    ...
    public var errorDescription: String? {
        ...
        case .activeWork: "Stop or transfer the employee's active work before changing their employment state."
...
public enum EmployeePackageCatalogue {
    public static func starterPackages(now: Date = Date()) -> [EmployeePackage] {
        let skills = Dictionary(uniqueKeysWithValues: OrganizationKnowledge.builtInSkills(now: now).map { ($0.id, EmployeePackageSkill($0)) })
        func selected(_ ids: [String]) -> [EmployeePackageSkill] { ids.compactMap { skills[$0] } }
        return [
            EmployeePackage(id: "starter.mira", version: "1.0.0", creator: "Agent Office", name: "Mira", role: "Executive Assistant", responsibility: "Keep the owner oriented, surface decisions, and prepare clear daily handoffs.", managerRole: "Owner", avatarColor: "B7A5D8", skills: selected(["executive-briefing", "communication"]), reducedModeDescription: "Can organize local company context without external connections.", builtIn: true),
            EmployeePackage(id: "starter.maya", version: "1.0.0", creator: "Agent Office", name: "Maya", role: "Editorial Manager", responsibility: "Own the content outcome, review work, and keep the team moving.", avatarColor: "E78B5B", skills: selected(["outcome-ownership", "editorial-review", "owner-reporting", "communication"]), builtIn: true),
            EmployeePackage(id: "starter.nia", version: "1.0.0", creator: "Agent Office", name: "Nia", role: "Audience Researcher", responsibility: "Understand the product and find useful questions worth answering.", managerRole: "Editorial Manager", avatarColor: "F2C96D", skills: selected(["audience-research", "communication"]), requiredConnectionIDs: ["web-research"], reducedModeDescription: "Can research supplied local material; live web research waits for an explicit grant.", builtIn: true),
            EmployeePackage(id: "starter.theo", version: "1.0.0", creator: "Agent Office", name: "Theo", role: "Content Writer", responsibility: "Turn evidence and direction into clear, useful articles.", managerRole: "Editorial Manager", avatarColor: "7395A8", skills: selected(["evidence-writing", "communication"]), builtIn: true),
            EmployeePackage(id: "starter.iris", version: "1.0.0", creator: "Agent Office", name: "Iris", role: "Customer Voice Analyst", responsibility: "Turn deliberately supplied customer feedback into one cited owner decision each week.", managerRole: "Executive Assistant", avatarColor: "6E8B62", skills: selected(["customer-voice-analysis", "communication"]), reducedModeDescription: "Works only from feedback deliberately placed in the local inbox.", builtIn: true),
        ]
    }
    ...
    public static func validate(_ package: EmployeePackage) throws {
        ...
        guard package.id.wholeMatch(of: identifierPattern) != nil else { throw EmployeePackageError.invalidIdentifier }
        guard package.version.wholeMatch(of: versionPattern) != nil else { throw EmployeePackageError.invalidVersion }
        guard !package.creator.trimmed.isEmpty, !package.name.trimmed.isEmpty, !package.role.trimmed.isEmpty, !package.responsibility.trimmed.isEmpty else {
        ...
        guard package.supportedProviders.contains(package.preferredProvider) else { throw EmployeePackageError.unsupportedProvider }
    }

    public static func decodeAndValidate(_ data: Data) throws -> EmployeePackage {
        let object: Any
        do { object = try JSONSerialization.jsonObject(with: data) } catch { throw EmployeePackageError.malformed }
        try inspect(object, keyPath: "package")
        var package: EmployeePackage
        do { package = try JSONDecoder().decode(EmployeePackage.self, from: data) } catch { throw EmployeePackageError.malformed }
    ...
    private static func inspect(_ value: Any, keyPath: String) throws {
        if let dictionary = value as? [String: Any] {
            for (key, nested) in dictionary {
                ...
                if ["secret", "token", "password", "credential", "apikey", "privatekey", "sessionkey"].contains(where: normalized.contains) {
                    throw EmployeePackageError.unsafeSecret("\(keyPath).\(key)")
                }
                try inspect(nested, keyPath: "\(keyPath).\(key)")
            }
        } else if let array = value as? [Any] {
            for (index, nested) in array.enumerated() { try inspect(nested, keyPath: "\(keyPath)[\(index)]") }
        } else if let string = value as? String {
            let lowered = string.lowercased()
            if lowered.hasPrefix("sk-") || lowered.contains("-----begin private key-----") {
                throw EmployeePackageError.unsafeSecret(keyPath)
            }
            if lowered.hasPrefix("/") || [".sh", ".command", ".app", ".dylib", ".so", ".exe"].contains(where: lowered.hasSuffix) {
...
public extension OrganizationState {
    var employeePackages: [EmployeePackage] { knowledge?.employeePackages ?? [] }
    var workingContracts: [WorkingContract] { knowledge?.workingContracts ?? [] }
    var contractChanges: [ContractChange] { knowledge?.contractChanges ?? [] }
    var supervisionEvents: [SupervisionEvent] { knowledge?.supervisionEvents ?? [] }

    var activeAIEmployees: [Employee] {
        employees.filter { $0.kind == .ai && [.hired, .paused].contains($0.effectiveEmploymentState) }
    }

    func workingContract(for employeeID: String) -> WorkingContract? {
        workingContracts.first { $0.employeeID == employeeID }
    }

    func employeePackage(id: String, version: String? = nil) -> EmployeePackage? {
        employeePackages.first { $0.id == id && (version == nil || $0.version == version) }
    }

    @discardableResult
    mutating func installEmployeePackage(_ package: EmployeePackage) throws -> Bool {
        try EmployeePackageCatalogue.validate(package)
        if employeePackages.contains(where: { $0.versionedID == package.versionedID }) { throw EmployeePackageError.duplicateVersion }
    ...
    @discardableResult
    mutating func removeEmployeePackage(id: String, version: String) throws -> Bool {
        guard let index = knowledge?.employeePackages.firstIndex(where: { $0.id == id && $0.version == version }) else { throw EmployeePackageError.missingPackage }
        guard !employees.contains(where: { $0.packageID == id && $0.packageVersion == version }) else { throw EmployeePackageError.packageInUse }
    ...
    mutating func hireEmployee(
        ...
        guard let package = employeePackage(id: packageID, version: version) else { throw EmploymentError.missingPackage }
        if let employed = employees.first(where: {
            $0.kind == .ai && $0.packageID == package.id && $0.packageVersion == package.version && [.hired, .paused].contains($0.effectiveEmploymentState)
        }) { return employed.id }
        if let candidateIndex = employees.firstIndex(where: {
            $0.kind == .ai && $0.packageID == package.id && $0.packageVersion == package.version && $0.effectiveEmploymentState == .candidate
        }) {
            ...
            recordEmployment(kind: .hire, employeeID: id, actorID: actorID, message: "Hired \(employees[candidateIndex].name) as \(employees[candidateIndex].role).", now: now)
            return id
        }
        let baseID = Self.employeeSlug(package.name)
        var employeeID = baseID
        var suffix = 2
        while employee(employeeID) != nil { employeeID = "\(baseID)-\(suffix)"; suffix += 1 }
        ...
        recordEmployment(kind: .hire, employeeID: employeeID, actorID: actorID, message: "Hired \(package.name) as \(package.role).", now: now)
        return employeeID
    }

    mutating func pauseEmployee(_ employeeID: String, actorID: String = "owner", reason: String = "Paused by the owner.", now: Date = Date()) throws {
        guard let index = employees.firstIndex(where: { $0.id == employeeID }) else { throw EmploymentError.missingEmployee }
        guard employees[index].kind == .ai else { throw EmploymentError.humanMember }
        guard employees[index].effectiveEmploymentState == .hired else { throw EmploymentError.notHired }
        employees[index].employmentState = .paused
        if let outcome = employeeOutcomes.first(where: { $0.assigneeID == employeeID && $0.status.isActivelyRunning }) {
            _ = updateEmployeeOutcome(outcome.id, now: now) { $0.status = .queued; $0.helpRequest = "Work was paused by the owner and is ready to resume." }
        }
        employees[index].status = .resting
        employees[index].currentTaskID = nil
        recordEmployment(kind: .pause, employeeID: employeeID, actorID: actorID, message: reason, now: now)
    }

    mutating func resumeEmployee(_ employeeID: String, actorID: String = "owner", reason: String = "Resumed by the owner.", now: Date = Date()) throws {
        guard let index = employees.firstIndex(where: { $0.id == employeeID }) else { throw EmploymentError.missingEmployee }
        guard employees[index].kind == .ai else { throw EmploymentError.humanMember }
        guard employees[index].effectiveEmploymentState == .paused else { throw EmploymentError.notHired }
        employees[index].employmentState = .hired
        recordEmployment(kind: .resume, employeeID: employeeID, actorID: actorID, message: reason, now: now)
    }

    mutating func retireEmployee(_ employeeID: String, actorID: String = "owner", reason: String = "Retired by the owner.", now: Date = Date()) throws {
        guard let index = employees.firstIndex(where: { $0.id == employeeID }) else { throw EmploymentError.missingEmployee }
        guard employees[index].kind == .ai else { throw EmploymentError.humanMember }
        guard employees[index].effectiveEmploymentState == .hired || employees[index].effectiveEmploymentState == .paused else { throw EmploymentError.notHired }
        guard !employeeOutcomes.contains(where: { $0.assigneeID == employeeID && $0.status.isActivelyRunning }) else { throw EmploymentError.activeWork }
        employees[index].employmentState = .retired
        employees[index].status = .resting
        employees[index].currentTaskID = nil
        recordEmployment(kind: .retire, employeeID: employeeID, actorID: actorID, message: reason, now: now)
    }

    mutating func updateWorkingContract(
        employeeID: String,
        role: String,
        responsibility: String,
        managerID: String?,
        ...
        guard let employeeIndex = employees.firstIndex(where: { $0.id == employeeID }), employees[employeeIndex].kind == .ai else { throw EmploymentError.missingEmployee }
        guard let contractIndex = knowledge?.workingContracts.firstIndex(where: { $0.employeeID == employeeID }) else { throw EmploymentError.notHired }
        ...
        revised.provenance = Self.contractFields.map { ContractFieldProvenance(field: $0, source: .organization) }
        ...
        knowledge?.skillAssignments.removeAll { $0.employeeID == employeeID && !revised.assignedSkillIDs.contains($0.skillID) }
        for skillID in revised.assignedSkillIDs where skill(skillID) != nil {
            if knowledge?.skillAssignments.contains(where: { $0.employeeID == employeeID && $0.skillID == skillID }) != true {
                knowledge?.skillAssignments.append(EmployeeSkillAssignment(id: "\(employeeID):\(skillID)", skillID: skillID, employeeID: employeeID, assignedByActorID: actorID, assignedAt: now))
            }
        }
        appendContractChanges(from: old, to: revised, actorID: actorID, reason: reason, now: now)
        recordEmployment(kind: .contractChanged, employeeID: employeeID, actorID: actorID, message: "Updated working contract revision \(revised.revision): \(reason)", now: now)
    }

    mutating func applyPackageUpdate(employeeID: String, packageID: String, version: String, actorID: String = "owner", now: Date = Date()) throws {
        guard let employeeIndex = employees.firstIndex(where: { $0.id == employeeID }) else { throw EmploymentError.missingEmployee }
        guard employees[employeeIndex].packageID == packageID else { throw EmploymentError.incompatiblePackage }
        guard let package = employeePackage(id: packageID, version: version) else { throw EmploymentError.missingPackage }
        guard let contractIndex = knowledge?.workingContracts.firstIndex(where: { $0.employeeID == employeeID }) else { throw EmploymentError.notHired }
        ...
        for packageSkill in package.skills where skill(packageSkill.id) == nil {
            knowledge?.skillDefinitions.append(packageSkill.materialized(authorID: package.creator, now: now))
        }
        recordEmployment(kind: .contractChanged, employeeID: employeeID, actorID: actorID, message: "Applied \(package.name) package \(version) without changing employee identity or grants.", now: now)
    }

    mutating func migrateEmployment(now: Date) {
        if knowledge == nil { knowledge = OrganizationKnowledge(productBrief: "") }
        for package in EmployeePackageCatalogue.starterPackages(now: now) where !employeePackages.contains(where: { $0.versionedID == package.versionedID }) {
            knowledge?.employeePackages.append(package)
        }
        let packageByEmployee = ["mira": "starter.mira", "maya": "starter.maya", "nia": "starter.nia", "theo": "starter.theo", "iris": "starter.iris"]
        for index in employees.indices {
            ...
            if employees[index].packageID == nil, let packageID = packageByEmployee[employees[index].id], let package = employeePackage(id: packageID) {
            ...
            let package = employees[index].packageID.flatMap { employeePackage(id: $0, version: employees[index].packageVersion) }
            try? materializeContract(for: employees[index].id, package: package, actorID: "agent-office-migration", now: now, source: .migration)
        }
        organizationConcurrencyLimit = effectiveConcurrencyLimit
    }

    private mutating func materializeContract(for employeeID: String, package: EmployeePackage?, actorID: String, now: Date, source: ContractValueSource = .employeePackage) throws {
        guard let employeeIndex = employees.firstIndex(where: { $0.id == employeeID }) else { throw EmploymentError.missingEmployee }
        if knowledge == nil { knowledge = OrganizationKnowledge(productBrief: "") }
        ...
        if let package {
            ...
            for packageSkill in package.skills where knowledge?.skillAssignments.contains(where: { $0.employeeID == employeeID && $0.skillID == packageSkill.id }) != true {
                knowledge?.skillAssignments.append(EmployeeSkillAssignment(id: "\(employeeID):\(packageSkill.id)", skillID: packageSkill.id, employeeID: employeeID, assignedByActorID: actorID, assignedAt: now))
            }
        }
        let skillIDs = assignedSkills(employeeID: employeeID).map(\.id)
        let connectionIDs = package?.requiredConnectionIDs ?? skillIDs.flatMap { skill($0)?.requiredConnectionIDs ?? [] }.uniqued
        let contract = WorkingContract(
            ...
            executionProvider: source == .migration ? EmployeeExecutionProvider(executionMode) : (package?.preferredProvider ?? EmployeeExecutionProvider(executionMode)),
    ...
    private mutating func recordEmployment(kind: SupervisionEventKind, employeeID: String, actorID: String, message: String, now: Date) {
        knowledge?.supervisionEvents.append(SupervisionEvent(kind: kind, actorID: actorID, employeeID: employeeID, message: message, createdAt: now))
        activity.append(Activity(id: UUID().uuidString, actorID: actorID, kind: kind == .hire ? .joined : .progress, message: message, createdAt: now))
    }

    private mutating func appendContractChanges(from old: WorkingContract, to new: WorkingContract, actorID: String, reason: String, now: Date) {
        let values: [(String, String, String)] = [
            ...
            ("skills", old.assignedSkillIDs.joined(separator: ","), new.assignedSkillIDs.joined(separator: ",")),
            ("connections", old.declaredConnectionIDs.joined(separator: ","), new.declaredConnectionIDs.joined(separator: ",")),
            ("grants", old.capabilityGrants.joined(separator: ","), new.capabilityGrants.joined(separator: ",")),
        ...
        for (field, previous, next) in values where previous != next {
            knowledge?.contractChanges.append(ContractChange(employeeID: new.employeeID, field: field, previousValue: previous, newValue: next, actorID: actorID, reason: reason, createdAt: now))
        }
    }

    private static let contractFields = ["identity", "role", "responsibility", "relationships", "skills", "connections", "grants", "executionProvider", "model", "environment", "workspace", "boundaries", "reviewPolicy"]
