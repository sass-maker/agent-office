...
extension OrganizationState {
  ...
  public mutating func beginDutyOccurrence(
    ...
    let canonicalOutcomeID = try createEmployeeOutcome(
      ...
      acceptanceCriteria: [
        "Account for supplied inputs, cite every decision basis, and recommend exactly one owner decision."
      ],
    ...
    knowledge?.dutyOccurrences.append(
      DutyOccurrence(
        id: occurrenceID,
        dutyID: duty.id,
  ...
  public mutating func resetInterruptedDuty(now: Date = Date()) -> Bool {
    ...
    knowledge!.dutyOccurrences[index].blockingReason =
