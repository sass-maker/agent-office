...
public struct EmployeeOutcomeRunRequest: Sendable {
    ...
    public init(organization: OrganizationState, outcomeID: String) throws {
        ...
        self.expectedTaskRevisions = Dictionary(uniqueKeysWithValues: outcome.taskIDs.compactMap { id in
...
public struct EmployeeOutcomeRunResult: Sendable {
    ...
    public init(request: EmployeeOutcomeRunRequest, initial: OrganizationState, result: OrganizationState) throws {
        guard let outcome = result.employeeOutcome(request.outcomeID), let employee = result.employee(request.employeeID) else { throw EmployeeOutcomeError.missingEmployee }
        ...
        self.memoryEntries = result.knowledge?.memoryEntries.filter { !initialMemoryIDs.contains($0.id) } ?? []
...
public enum EmployeeRunApplyError: LocalizedError, Equatable {
    ...
    public var errorDescription: String? {
        switch self {
        case .outcomeChanged: "The outcome changed while this result was running. Its stale result was not applied."
        case .taskChanged(let id): "Ticket \(id) changed while it was running. Its stale result was not applied."
        case .contractChanged: "The employee's working contract changed while work was running. Review and resume the outcome."
        }
    }
}

public extension OrganizationState {
    mutating func apply(_ result: EmployeeOutcomeRunResult) throws {
        guard let outcomeIndex = knowledge?.employeeOutcomes.firstIndex(where: { $0.id == result.outcomeID }),
        ...
        if result.contractRevision > 0, workingContract(for: result.employeeID)?.revision != result.contractRevision {
            throw EmployeeRunApplyError.contractChanged
        }
        for (taskID, expectedRevision) in result.expectedTaskRevisions {
            guard task(taskID)?.effectiveWorkRevision == expectedRevision else { throw EmployeeRunApplyError.taskChanged(taskID) }
        }
        knowledge!.employeeOutcomes[outcomeIndex] = result.outcome
        for resultTask in result.tasks {
            if let index = tasks.firstIndex(where: { $0.id == resultTask.id }) { tasks[index] = resultTask } else { tasks.append(resultTask) }
        }
        for artifact in result.artifacts where !artifacts.contains(where: { $0.id == artifact.id }) { artifacts.append(artifact) }
        for blocker in result.blockers {
            if let index = blockers.firstIndex(where: { $0.id == blocker.id }) { blockers[index] = blocker } else { blockers.append(blocker) }
        }
        activity.append(contentsOf: result.activity.filter { event in !activity.contains(where: { $0.id == event.id }) })
        if knowledge == nil { knowledge = OrganizationKnowledge(productBrief: "") }
        let existingMemoryIDs = Set(knowledge?.memoryEntries.map(\.id) ?? [])
        knowledge?.memoryEntries.append(contentsOf: result.memoryEntries.filter { !existingMemoryIDs.contains($0.id) })
...
public actor EmployeeWorkCoordinator {
    public typealias Operation = @Sendable (EmployeeOutcomeRunRequest) async throws -> EmployeeOutcomeRunResult
    public typealias Completion = @MainActor @Sendable (Result<EmployeeOutcomeRunResult, Error>) async -> Void
    ...
    public func submit(_ request: EmployeeOutcomeRunRequest, operation: @escaping Operation, completion: @escaping Completion) -> Bool {
        guard tasksByEmployee[request.employeeID] == nil, tasksByEmployee.count < concurrencyLimit else { return false }
        tasksByEmployee[request.employeeID] = Task { [weak self] in
            let result: Result<EmployeeOutcomeRunResult, Error>
            do { result = .success(try await operation(request)) }
...
public extension EmployeeOutcomeEngine {
    func execute(
        ...
        let result = await run(request.organization, outcomeID: request.outcomeID, runner: runner, store: store, now: now, persistsTransitions: false)
