...
public struct FeedbackInputSnapshot: Sendable, Equatable {
    ...
    public var promptContext: String {
        ...
        let excluded = exclusions.isEmpty
            ? "None."
    ...
    public static func capture(
        ...
        for url in entries {
            ...
            if values.isHidden == true || name.hasPrefix(".") {
                exclusions.append(DutyInputExclusion(fileName: name, reason: "Hidden entries are not analyzed."))
                continue
            }
            if values.isSymbolicLink == true {
                exclusions.append(DutyInputExclusion(fileName: name, reason: "Symbolic links are outside the inbox boundary."))
                continue
            }
            guard values.isRegularFile == true else {
                exclusions.append(DutyInputExclusion(fileName: name, reason: "Only direct regular files are analyzed."))
                continue
            }
            ...
            guard let content = String(data: data, encoding: .utf8) else {
                exclusions.append(DutyInputExclusion(fileName: name, reason: "The file is not valid UTF-8 text."))
                continue
            }

            let label = "F\(files.count + 1)"
            files.append(FeedbackInputFile(
...
public enum CustomerVoiceEvidenceVerifier {
    public static func hasRequiredSections(_ content: String) -> Bool {
        let headings = content.split(separator: "\n")
            ...
            .filter { $0.hasPrefix("#") }
        return ["input coverage", "themes", "evidence", "uncertainty", "owner decision", "next occurrence"]
...
public struct CustomerVoiceDutyEngine: Sendable {
    ...
    public func run(
        ...
        do {
            ...
            guard CustomerVoiceEvidenceVerifier.containsValidSourceLabel(
                output.content,
                references: snapshot.references
            ) else { throw CustomerVoiceDutyError.missingSourceLabel }

            let completedAt = Date()
            let evidenceBasis = state.executionMode == .localCodex
            ...
            let notice = state.executionMode == .localCodex
            ...
            _ = state.updateDuty(duty.id) { value in
                ...
                value.nextDueAt = Calendar.current.date(byAdding: .day, value: 7, to: value.nextDueAt)
    ...
    private func memoryContext(for employeeID: String, in state: OrganizationState) -> String {
        let entries = state.knowledge?.memoryEntries
