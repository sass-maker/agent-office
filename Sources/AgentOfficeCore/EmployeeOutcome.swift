...
public struct OutcomeManagementMessage: Identifiable, Codable, Sendable, Equatable {
  ...
  public init(
    id: String = UUID().uuidString, actorID: String, message: String, taskID: String? = nil,
    createdAt: Date
  ) {
    self.id = id
...
public struct OutcomeDelivery: Identifiable, Codable, Sendable, Equatable {
  ...
  public init(
    id: String = UUID().uuidString, summary: String, artifactIDs: [String], evidenceBasis: String,
    limitations: String, recommendedNextAction: String, deliveredByEmployeeID: String,
    createdAt: Date
  ) {
    self.id = id
...
public struct OutcomeRevision: Identifiable, Codable, Sendable, Equatable {
  ...
  public init(
    id: String = UUID().uuidString, feedback: String, requestedByActorID: String, taskID: String,
    createdAt: Date
  ) {
    self.id = id
...
extension OrganizationState {
  ...
  public var activeEmployeeOutcome: EmployeeOutcome? {
    employeeOutcomes.last { !$0.status.isTerminal && $0.status != .delivered }
  }

  public var latestEmployeeOutcome: EmployeeOutcome? {
    employeeOutcomes.max { $0.createdAt < $1.createdAt }
  }

  public func latestEmployeeOutcome(for employeeID: String) -> EmployeeOutcome? {
    employeeOutcomes.filter { $0.assigneeID == employeeID }.max { $0.createdAt < $1.createdAt }
  }

  public func activeEmployeeOutcome(for employeeID: String) -> EmployeeOutcome? {
    employeeOutcomes
  ...
  public func queuedEmployeeOutcomes(for employeeID: String) -> [EmployeeOutcome] {
    employeeOutcomes.filter {
      $0.assigneeID == employeeID && [.queued, .approved, .revision].contains($0.status)
    }
    .sorted(by: Self.outcomeQueueOrder)
  }

  public func employeeOutcome(_ id: String) -> EmployeeOutcome? {
    employeeOutcomes.first { $0.id == id }
  }

  @discardableResult
  public mutating func updateEmployeeOutcome(
    _ id: String,
    now: Date = Date(),
  ...
  @discardableResult
  public mutating func createEmployeeOutcome(
    employeeID: String,
    outcome: String,
    context: String,
    acceptanceCriteria: [String] = [],
    ...
    guard employee.effectiveEmploymentState != .paused else {
      throw EmployeeOutcomeError.employeePaused
    }
    guard employee.effectiveEmploymentState == .hired else {
      throw EmployeeOutcomeError.employeeNotHired
    }
    guard !assignedSkills(employeeID: employeeID).isEmpty else {
      throw EmployeeOutcomeError.noAssignedSkills
    }
    ...
    knowledge?.employeeOutcomes.append(
      EmployeeOutcome(
        id: id,
    ...
    activity.append(
      Activity(
        ...
        kind: .handoff,
        message:
          "I’ll own this outcome. I’m choosing the right skills and turning it into a short plan.",
        createdAt: now
      ))
    return id
  }
  ...
  @discardableResult
  public mutating func cancelEmployeeOutcome(_ id: String, now: Date = Date()) -> Bool {
    ...
    for taskID in outcome.taskIDs {
      guard let index = tasks.firstIndex(where: { $0.id == taskID }), tasks[index].status != .done
      else { continue }
  ...
  @discardableResult
  public mutating func retryEmployeeOutcome(_ id: String, now: Date = Date()) -> Bool {
    ...
    resolveBlockersAndUnblockTasks(for: outcome.taskIDs, now: now)
    return true
  }

  private mutating func resolveBlockersAndUnblockTasks(for taskIDs: [String], now: Date) {
    for index in blockers.indices where taskIDs.contains(blockers[index].taskID) {
      blockers[index].resolved = true
    }
    for taskID in taskIDs {
      ...
      tasks[index].status =
        tasks[index].dependencyIDs.allSatisfy { dependencyID in
  ...
  @discardableResult
  public mutating func resetInterruptedEmployeeOutcome(now: Date = Date()) -> Bool {
    ...
    for outcome in interrupted {
      ...
      activity.append(
        Activity(
          id: UUID().uuidString, actorID: outcome.assigneeID, kind: .stopped,
          message:
            "My previous run stopped before delivery. I kept the plan and completed tickets ready to resume.",
          createdAt: now))
    }
    return true
  }
  ...
  private static func outcomeQueueOrder(_ lhs: EmployeeOutcome, _ rhs: EmployeeOutcome) -> Bool {
    if lhs.effectivePriority.rank != rhs.effectivePriority.rank {
      return lhs.effectivePriority.rank < rhs.effectivePriority.rank
    }
