...
public struct EmployeeSkillAssignment: Identifiable, Codable, Sendable, Equatable {
    ...
    public init(id: String, skillID: String, employeeID: String, assignedByActorID: String, assignedAt: Date) {
        self.id = id
...
public struct ConnectionDefinition: Identifiable, Codable, Sendable, Equatable {
    ...
    public init(id: String, name: String, kind: ConnectionKind, summary: String, capabilityID: String? = nil) {
        self.id = id
        self.name = name
        self.kind = kind
        self.summary = summary
...
public struct OrganizationKnowledge: Codable, Sendable, Equatable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        memoryEntries = try container.decodeIfPresent([EmployeeMemoryEntry].self, forKey: .memoryEntries) ?? []
        capabilityEvents = try container.decodeIfPresent([CapabilityEvent].self, forKey: .capabilityEvents) ?? []
        assistantHandoffs = try container.decodeIfPresent([AssistantHandoff].self, forKey: .assistantHandoffs) ?? []
        skillDefinitions = try container.decodeIfPresent([SkillDefinition].self, forKey: .skillDefinitions) ?? []
        skillAssignments = try container.decodeIfPresent([EmployeeSkillAssignment].self, forKey: .skillAssignments) ?? []
        connectionDefinitions = try container.decodeIfPresent([ConnectionDefinition].self, forKey: .connectionDefinitions) ?? []
        researchAssignments = try container.decodeIfPresent([ResearchAssignment].self, forKey: .researchAssignments) ?? []
...
public struct OrganizationState: Codable, Sendable, Equatable {
        ...
        return OrganizationState(
            ...
            knowledge: OrganizationKnowledge(
                ...
                profile: OrganizationProfile(
                    purpose: "Build a useful body of work that helps the right people understand the product.",
                    product: "A prepared local studio for rehearsing a small content team's first mission.",
                    audience: "Early product teams learning how named AI employees can coordinate useful work.",
                    stage: "Proof of concept",
                    operatingPrinciples: "Stay grounded in supplied facts. Prefer useful evidence to content volume. Keep owner decisions explicit.",
    ...
    public mutating func createResearchAssignment(
        ...
        guard activeResearchAssignment == nil else { throw ResearchAssignmentError.activeAssignmentExists }
    ...
    public mutating func resetInterruptedResearch(now: Date = Date()) -> Bool {
        guard let index = knowledge?.researchAssignments.lastIndex(where: { $0.status == .researching }) else {
            return false
        }
        knowledge?.researchAssignments[index].status = .queued
        knowledge?.researchAssignments[index].blockingReason = "The previous run stopped before delivery. It is ready to resume."
        knowledge?.researchAssignments[index].updatedAt = now
        activity.append(Activity(
            id: UUID().uuidString,
            actorID: "mira",
    ...
    public mutating func cancelResearchAssignment(
        ...
        guard let assignment = researchAssignment(id), !assignment.status.isTerminal else { return false }
    ...
    public func assignedSkills(employeeID: String) -> [SkillDefinition] {
        let skillIDs = Set(knowledge?.skillAssignments.filter { $0.employeeID == employeeID }.map(\.skillID) ?? [])
    ...
    public func employeesWithSkill(_ skillID: String) -> [Employee] {
        let employeeIDs = Set(knowledge?.skillAssignments.filter { $0.skillID == skillID }.map(\.employeeID) ?? [])
    ...
    public mutating func assignSkill(
        ...
        if knowledge?.skillAssignments.contains(where: { $0.skillID == skillID && $0.employeeID == employeeID }) == true {
            return false
        }
        if knowledge == nil { knowledge = OrganizationKnowledge(productBrief: "") }
        knowledge?.skillAssignments.append(EmployeeSkillAssignment(
    ...
    public mutating func teachSkill(
        ...
        knowledge?.skillDefinitions.append(SkillDefinition(
            id: id,
            name: values[0],
            category: category.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? "Organization" : category.trimmingCharacters(in: .whitespacesAndNewlines),
            purpose: values[1],
    ...
    public mutating func applyOnboarding(
        ...
        if !trimmedOwnerName.isEmpty,
           let ownerIndex = employees.firstIndex(where: { $0.id == "owner" }) {
            ...
            if let assistantIndex = employees.firstIndex(where: { $0.assistantForHumanID == "owner" }) {
                employees[assistantIndex].responsibility = "Keep \(trimmedOwnerName) oriented, surface decisions, and prepare clear daily handoffs."
            }
        }
        ...
        if let niaIndex = employees.firstIndex(where: { $0.id == "nia" }) {
            ...
            if hadGrant != webResearchGranted {
                knowledge?.capabilityEvents.append(CapabilityEvent(
                    id: UUID().uuidString,
...
extension OrganizationKnowledge {
    public static func builtInSkills(now: Date = Date()) -> [SkillDefinition] {
        [
            SkillDefinition(id: "executive-briefing", name: "Executive briefing", category: "Coordination", purpose: "Keep the owner oriented around current work and decisions.", instructions: "Summarize only persisted goals, tasks, blockers, permissions, and artifacts. Separate completed work from pending work and end with the next owner decision.", successCriteria: "The brief is attributable, contains no invented progress, and names one clear next action.", source: .builtIn, authorID: "agent-office", createdAt: now, updatedAt: now),
            SkillDefinition(id: "outcome-ownership", name: "Outcome ownership", category: "Management", purpose: "Own a bounded organizational outcome and coordinate specialists toward it.", instructions: "Break the outcome into the existing bounded tasks, keep ownership explicit, review handoffs, and stop at the configured revision limit.", successCriteria: "The outcome ends with approved work or a precise owner blocker, never an infinite review loop.", source: .builtIn, authorID: "agent-office", createdAt: now, updatedAt: now),
            SkillDefinition(id: "audience-research", name: "Audience research", category: "Research", purpose: "Find the useful audience question that the content should answer.", instructions: "Ground the question in the product brief. Use external sources only when web research is granted, cite URLs when used, and distinguish evidence from owner claims.", successCriteria: "Research identifies a concrete reader question, evidence basis, and article direction without fabricated sources.", source: .builtIn, requiredConnectionIDs: ["web-research"], authorID: "agent-office", createdAt: now, updatedAt: now),
            SkillDefinition(id: "evidence-writing", name: "Evidence-based writing", category: "Content", purpose: "Turn approved evidence and direction into a useful article.", instructions: "Write from the product brief and research artifact, preserve claim boundaries, and make the result independently readable rather than summarizing the transcript.", successCriteria: "The draft answers the audience question, traces its evidence, and is ready for editorial review.", source: .builtIn, authorID: "agent-office", createdAt: now, updatedAt: now),
            SkillDefinition(id: "editorial-review", name: "Editorial review", category: "Quality", purpose: "Evaluate content against its outcome, evidence, and safe claims.", instructions: "Return a clear approve or revise verdict with focused evidence. Do not request more than the bounded revision limit permits.", successCriteria: "The verdict is actionable, grounded in the artifact, and either approves the work or identifies one bounded correction.", source: .builtIn, authorID: "agent-office", createdAt: now, updatedAt: now),
            SkillDefinition(id: "owner-reporting", name: "Owner reporting", category: "Coordination", purpose: "Leave the owner a concise account of completed work and next decisions.", instructions: "Summarize completed tasks, created artifacts, unresolved blockers, capability use, and the recommended next action.", successCriteria: "The report matches persisted state and lets the owner resume without reading the full activity timeline.", source: .builtIn, authorID: "agent-office", createdAt: now, updatedAt: now),
            SkillDefinition(id: "customer-voice-analysis", name: "Customer voice analysis", category: "Research", purpose: "Turn a bounded set of supplied customer evidence into one traceable weekly decision.", instructions: "Account for every included source, group repeated themes without overstating prevalence, cite source labels for each claim, state uncertainty, and recommend exactly one owner decision.", successCriteria: "The brief reports coverage, traces themes to supplied files, distinguishes single observations from repeated evidence, and recommends one bounded decision.", source: .builtIn, authorID: "agent-office", createdAt: now, updatedAt: now),
            SkillDefinition(id: "communication", name: "Communication", category: "Coordination", purpose: "Keep work supervisable through concise, attributable updates and precise requests for help.", instructions: "Acknowledge the outcome, announce the ticket plan, report meaningful progress, ask one precise question when blocked, and finish with what changed, the artifacts created, and the recommended next action.", successCriteria: "The owner can understand acceptance, plan, progress, blockers, and delivery from persisted activity without reading a raw model transcript.", source: .builtIn, authorID: "agent-office", createdAt: now, updatedAt: now),
        ]
    }

    public static func builtInAssignments(now: Date = Date()) -> [EmployeeSkillAssignment] {
        [
            EmployeeSkillAssignment(id: "mira:executive-briefing", skillID: "executive-briefing", employeeID: "mira", assignedByActorID: "agent-office", assignedAt: now),
            EmployeeSkillAssignment(id: "maya:outcome-ownership", skillID: "outcome-ownership", employeeID: "maya", assignedByActorID: "agent-office", assignedAt: now),
            EmployeeSkillAssignment(id: "nia:audience-research", skillID: "audience-research", employeeID: "nia", assignedByActorID: "agent-office", assignedAt: now),
            EmployeeSkillAssignment(id: "theo:evidence-writing", skillID: "evidence-writing", employeeID: "theo", assignedByActorID: "agent-office", assignedAt: now),
            EmployeeSkillAssignment(id: "maya:editorial-review", skillID: "editorial-review", employeeID: "maya", assignedByActorID: "agent-office", assignedAt: now),
            EmployeeSkillAssignment(id: "maya:owner-reporting", skillID: "owner-reporting", employeeID: "maya", assignedByActorID: "agent-office", assignedAt: now),
            EmployeeSkillAssignment(id: "iris:customer-voice-analysis", skillID: "customer-voice-analysis", employeeID: "iris", assignedByActorID: "agent-office", assignedAt: now),
            EmployeeSkillAssignment(id: "mira:communication", skillID: "communication", employeeID: "mira", assignedByActorID: "agent-office", assignedAt: now),
            EmployeeSkillAssignment(id: "maya:communication", skillID: "communication", employeeID: "maya", assignedByActorID: "agent-office", assignedAt: now),
            EmployeeSkillAssignment(id: "nia:communication", skillID: "communication", employeeID: "nia", assignedByActorID: "agent-office", assignedAt: now),
            EmployeeSkillAssignment(id: "theo:communication", skillID: "communication", employeeID: "theo", assignedByActorID: "agent-office", assignedAt: now),
            EmployeeSkillAssignment(id: "iris:communication", skillID: "communication", employeeID: "iris", assignedByActorID: "agent-office", assignedAt: now),
        ]
    }

    public static func builtInConnections() -> [ConnectionDefinition] {
        [
            ConnectionDefinition(id: "demo-runner", name: "Demo team", kind: .execution, summary: "Local deterministic rehearsal with no account or network."),
            ConnectionDefinition(id: "local-codex", name: "Local Codex", kind: .execution, summary: "Uses the locally installed and authenticated Codex CLI in a read-only sandbox."),
            ConnectionDefinition(id: "web-research", name: "Web research", kind: .tool, summary: "Read-only live research for explicitly permitted research work.", capabilityID: "web-research"),
        ]
    }
}
