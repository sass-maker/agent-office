...
public actor LocalOrganizationStore {
  ...
  public func importEmployeePackage(from sourceURL: URL, into input: OrganizationState) throws
    -> OrganizationState
  {
    ...
    let destination = employeePackagesURL.appendingPathComponent(
      Self.packageFilename(package), isDirectory: false)
  ...
  public func removeEmployeePackage(id: String, version: String, from input: OrganizationState)
    throws -> OrganizationState
  {
    var organization = input
    try organization.removeEmployeePackage(id: id, version: version)
    let package = EmployeePackage(
      id: id, version: version, creator: "", name: "", role: "", responsibility: "",
      avatarColor: "", skills: []
    )
    let destination = employeePackagesURL.appendingPathComponent(
      Self.packageFilename(package), isDirectory: false)
  ...
  private func materialize(_ organization: OrganizationState) throws {
    ...
    let members = organization.employees.map { employee in
      ...
      let assistant =
        employee.assistantForHumanID.map {
          " · paired with \(organization.employee($0)?.name ?? $0)"
        } ?? ""
      return
        "- **\(employee.name)** · \(employee.kind == .human ? "Human" : "AI") · \(employee.role) · reports to \(manager)\(assistant)"
    }.joined(separator: "\n")
    try """
    # \(organization.name)
    ...
    let skillCatalogue = (organization.knowledge?.skillDefinitions ?? [])
      ...
      .map { skill in
        ...
        let connections =
          skill.requiredConnectionIDs.isEmpty
          ? "None" : skill.requiredConnectionIDs.joined(separator: ", ")
        return """
    ...
    let dutyHistory = organization.employeeDuties.map { duty in
      ...
      if let latest {
        coverage =
          "\(latest.includedInputs.count) included · \(latest.excludedInputs.count) excluded"
      } else {
    ...
    let outcomeHistory = organization.employeeOutcomes
      ...
      .map { outcome in
        ...
        let tickets = outcome.taskIDs.compactMap { taskID in
          organization.task(taskID).map {
            "- [\($0.status == .done ? "x" : " ")] \($0.title) · `\($0.status.rawValue)`"
          }
        }.joined(separator: "\n")
    ...
    let packageCatalogue = organization.employeePackages
      ...
      .map { package in
        ...
        let connections =
          ...
          ? "None" : package.requiredConnectionIDs.joined(separator: ", ")
        return """
  ...
  private func materializeEmployeeHomes(for organization: OrganizationState) throws {
    for employee in organization.employees where employee.kind == .ai {
      ...
      """.write(
        to: home.appendingPathComponent("RESPONSIBILITIES.md"), atomically: true, encoding: .utf8)

      let memories =
        organization.knowledge?.memoryEntries
      ...
      let memoryBody =
        ...
        : memories.map { entry in
          ...
          return
            "- Day \(entry.dayNumber) · \(entry.createdAt.formatted(.iso8601)) · by `\(entry.authorID)`\(source)\n  \(entry.summary)"
        }.joined(separator: "\n")
      try "# Memory\n\n\(memoryBody)\n".write(
      ...
      let grantBody =
        grants.isEmpty
        ? "No external capabilities granted." : grants.map { "- `\($0)`" }.joined(separator: "\n")
      ...
      let artifactBody =
        ...
        ? "No artifacts yet."
        : artifacts.map {
          "- [\($0.title)](../../\($0.relativePath)) · \($0.kind.rawValue) · day \(organization.dayNumber)"
        }.joined(separator: "\n")
      try "# Artifacts\n\n\(artifactBody)\n".write(
  ...
  private func writeWorkingContract(
    for employee: Employee, contract: WorkingContract, to home: URL
  ) throws {
    let sources = Dictionary(
      uniqueKeysWithValues: contract.provenance.map { ($0.field, $0.source.rawValue) })
    let skillList =
      contract.assignedSkillIDs.isEmpty
      ? "None" : contract.assignedSkillIDs.map { "`\($0)`" }.joined(separator: ", ")
    let declared =
      contract.declaredConnectionIDs.isEmpty
      ? "None" : contract.declaredConnectionIDs.map { "`\($0)`" }.joined(separator: ", ")
    let grants =
      contract.capabilityGrants.isEmpty
      ? "None" : contract.capabilityGrants.map { "`\($0)`" }.joined(separator: ", ")
    try """
    # Working contract
  ...
  private nonisolated static func starterProfile(for organization: OrganizationState)
    -> OrganizationProfile
  {
    ...
    return OrganizationProfile(
      ...
      stage: "Early stage",
      operatingPrinciples:
        "Stay grounded in supplied facts and bring the owner in when judgment is required.",
      constraints:
        "Respect the current local-only permissions and do not invent unsupported claims."
    )
  }
