...
final class EmployeeWorkCoordinatorTests: XCTestCase {
  func testCoordinatorRunsDifferentEmployeesWithinCapacityAndSerializesSameEmployee() async throws {
    ...
    let duplicateEmployee = await coordinator.submit(
      requests[0], operation: { try Self.syntheticResult($0) }, completion: { _ in })
    ...
    let overCapacity = await coordinator.submit(
      requests[2], operation: { try Self.syntheticResult($0) }, completion: { _ in })
  ...
  func testCancellingOneEmployeeDoesNotCancelAnother() async throws {
    ...
    for request in requests {
      let submitted = await coordinator.submit(
        request,
        operation: { request in
  ...
  private func makeRequests(employeeIDs: [String]) throws -> [EmployeeOutcomeRunRequest] {
    ...
    for employeeID in employeeIDs {
      ids.append(
        try state.createEmployeeOutcome(
          employeeID: employeeID, outcome: "Outcome for \(employeeID)", context: ""))
    }
  ...
  private static func syntheticResult(_ request: EmployeeOutcomeRunRequest) throws
