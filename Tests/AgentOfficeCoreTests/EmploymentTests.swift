...
final class EmploymentTests: XCTestCase {
    func testFreshOrganizationStartsWithCandidatesAndHiringMaterializesContract() throws {
        ...
        var organization = LocalOrganizationStore.migrated(.seeded(now: now, hiredStarterTeam: false), now: now)
        ...
        let employeeID = try organization.hireEmployee(packageID: "starter.nia", actorID: "owner", now: now)
        ...
        XCTAssertEqual(organization.workingContract(for: "nia")?.assignedSkillIDs.sorted(), ["audience-research", "communication"])
        XCTAssertTrue(organization.workingContract(for: "nia")?.capabilityGrants.isEmpty == true)
        XCTAssertTrue(organization.supervisionEvents.contains { $0.kind == .hire && $0.employeeID == "nia" })
    }
    ...
    func testLegacyMigrationIsIdempotentAndPreservesHiredIdentity() {
        ...
        let migratedAgain = LocalOrganizationStore.migrated(migrated, now: Date(timeIntervalSince1970: 200))
    ...
    func testPackageValidationRejectsSecretsExecutablesAndDuplicates() throws {
        ...
        XCTAssertThrowsError(try EmployeePackageCatalogue.decodeAndValidate(secretData)) {
            guard case .unsafeSecret = $0 as? EmployeePackageError else { return XCTFail("Expected unsafe secret, got \($0)") }
        }
        ...
        XCTAssertThrowsError(try EmployeePackageCatalogue.decodeAndValidate(data)) {
            guard case .executableReference = $0 as? EmployeePackageError else { return XCTFail("Expected executable reference, got \($0)") }
        }
    ...
    func testPauseResumeRetirePreserveHistoryAndBlockActiveRetirement() throws {
        ...
        let outcomeID = try organization.createEmployeeOutcome(employeeID: "theo", outcome: "Write a note", context: "", now: now)
    ...
    func testWorkingContractProjectionContainsIdentifiersButNoCredentials() async throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("AgentOfficeEmploymentTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let store = LocalOrganizationStore(rootURL: root)
        let organization = LocalOrganizationStore.migrated(.seeded(now: Date(timeIntervalSince1970: 100)))

        try await store.save(organization)
        let contract = try String(contentsOf: root.appendingPathComponent("employees/nia/WORKING_CONTRACT.md"), encoding: .utf8)
