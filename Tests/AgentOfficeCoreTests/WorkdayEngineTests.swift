...
final class WorkdayEngineTests: XCTestCase {
    func testSeededOrganizationHasDurableEmployeeShape() {
        ...
        XCTAssertEqual(organization.employees.map(\.id), ["owner", "mira", "maya", "nia", "theo", "iris"])
        ...
        XCTAssertEqual(organization.assignedSkills(employeeID: "maya").map(\.id), ["communication", "editorial-review", "outcome-ownership", "owner-reporting"])
        ...
        grounded.knowledge?.productBrief = String(repeating: "Describe what this product does. ", count: 8)
    ...
    func testOnboardingAppliesCompanyModeAndPermissionToOnePersistableSnapshot() async throws {
        ...
        organization.applyOnboarding(
            ...
            productBrief: "A complete grounded product brief for Juniper House and its first bounded mission.",
            profile: profile,
    ...
    func testCompletedFirstUsePathSurvivesQuitAndReopen() async throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("employees/mira/IDENTITY.md").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("employees/nia/MEMORY.md").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("employees/maya/ARTIFACTS.md").path))
    }
    ...
    func testPersistenceRoundTripKeepsOrganizationAndArtifactsInspectable() async throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("COMPANY.md").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("PRODUCT_BRIEF.md").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("SKILLS.md").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("CONNECTIONS.md").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("RESEARCH_ASSIGNMENTS.md").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("employees/maya/IDENTITY.md").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("employees/maya/MEMORY.md").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("employees/maya/CAPABILITIES.md").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("employees/maya/SKILLS.md").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: root.appendingPathComponent("employees/maya/ARTIFACTS.md").path))
    }
    ...
    func testMigrationAddsOwnerAssistantAndKnowledgeWithoutLosingWork() {
        ...
        XCTAssertEqual(migratedAgain.employeeDuties.filter { $0.id == "customer-voice-weekly" }.count, 1)
    ...
    func testOwnerCanTeachPersistAndAssignSkillWithoutDuplicates() async throws {
        ...
        let skillID = try organization.teachSkill(
            ...
            instructions: "Separate observed statements from interpretation, group repeated needs, and retain contradictory evidence.",
            successCriteria: "The result names themes, supporting observations, contradictions, and unanswered questions.",
            employeeID: "nia",
            now: Date(timeIntervalSince1970: 200)
        )
        ...
        XCTAssertFalse(organization.assignSkill(skillID: skillID, employeeID: "nia", now: Date(timeIntervalSince1970: 201)))
        XCTAssertTrue(organization.assignSkill(skillID: skillID, employeeID: "theo", now: Date(timeIntervalSince1970: 202)))
        ...
        let catalogue = try String(contentsOf: root.appendingPathComponent("SKILLS.md"), encoding: .utf8)
        let niaSkills = try String(contentsOf: root.appendingPathComponent("employees/nia/SKILLS.md"), encoding: .utf8)
    ...
    func testTeachingRejectsIncompleteGuidance() {
        var organization = OrganizationState.seeded(now: Date(timeIntervalSince1970: 100))
        XCTAssertThrowsError(try organization.teachSkill(
            name: "",
            category: "Research",
    ...
    func testEmployeeHomesStayInsideSelectedOrganization() async throws {
        ...
        let firstIdentity = try String(contentsOf: firstRoot.appendingPathComponent("employees/maya/IDENTITY.md"), encoding: .utf8)
        let secondIdentity = try String(contentsOf: secondRoot.appendingPathComponent("employees/maya/IDENTITY.md"), encoding: .utf8)
        XCTAssertEqual(firstIdentity, secondIdentity)
        XCTAssertNotEqual(firstStore.rootURL, secondStore.rootURL)
        XCTAssertFalse(FileManager.default.fileExists(atPath: firstRoot.appendingPathComponent("employees/unknown").path))
    }
    ...
    func testAssistantBriefIsGroundedAndInterruptedHandoffIsDeduplicated() {
        ...
        organization.dayNumber = 1
        organization.workdayStatus = .active
        ExecutiveAssistant.appendInterruptedHandoff(to: &organization, now: Date(timeIntervalSince1970: 200))
        ExecutiveAssistant.appendInterruptedHandoff(to: &organization, now: Date(timeIntervalSince1970: 300))
    ...
    func testGrantedResearchRecordsCapabilitySuccessAndEvidence() async {
        ...
        organization.workdayStatus = .active
        organization.employees[organization.employees.firstIndex { $0.id == "nia" }!].capabilityGrants = ["web-research"]
        ...
        XCTAssertEqual(result.knowledge?.memoryEntries.first?.sourceArtifactID, result.artifacts.first?.id)
    }

    func testUnavailableAndFailedResearchBecomeAttributedBlockers() async {
        for (runner, expectedKind) in [
            (AnyEmployeeRunner { _ in throw CodexRunnerError.unavailable }, CapabilityEventKind.unavailable),
