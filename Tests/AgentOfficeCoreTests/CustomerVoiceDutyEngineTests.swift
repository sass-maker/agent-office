...
final class CustomerVoiceDutyEngineTests: XCTestCase {
  func testSeedAndMigrationAddIrisDutyAndSkillOnce() {
    ...
    XCTAssertEqual(
      migratedAgain.employeeDuties.filter { $0.id == CustomerVoiceDutyEngine.dutyID }.count, 1)
  }
  ...
  func testEmptyInboxBlocksBeforeRunner() async throws {
    ...
    XCTAssertTrue(
      result.dutyOccurrence(occurrenceID)?.blockingReason?.contains("Add a .txt") == true)
  ...
  func testDemoDutyDeliversProjectsAndAdvancesOnce() async throws {
    ...
    try write(
      "Onboarding was confusing, but the local files felt trustworthy.",
      to: store.feedbackInboxURL.appendingPathComponent("founder-note.md"))
    var organization = OrganizationState.seeded(now: Date(timeIntervalSince1970: 100))
    organization.executionMode = .demo
    let originalDue = try XCTUnwrap(
      organization.employeeDuty(CustomerVoiceDutyEngine.dutyID)?.nextDueAt)
    let occurrenceID = try organization.beginDutyOccurrence(
      dutyID: CustomerVoiceDutyEngine.dutyID,
      now: Date(timeIntervalSince1970: 200)
    )
    ...
    XCTAssertTrue(
      FileManager.default.fileExists(atPath: root.appendingPathComponent("DUTIES.md").path))
    XCTAssertTrue(
      FileManager.default.fileExists(atPath: root.appendingPathComponent("feedback-inbox").path))
    XCTAssertTrue(
      try String(contentsOf: root.appendingPathComponent("DUTIES.md"), encoding: .utf8).contains(
        "Open Iris's brief"))
  ...
  func testNextWeekCreatesAndDeliversADistinctOccurrence() async throws {
    ...
    try await store.ensureFeedbackInbox()
    try write(
      "The weekly setup still needs a clearer next action.",
      to: store.feedbackInboxURL.appendingPathComponent("weekly-note.md"))
  ...
  func testRealDutyRequiresValidCapturedSourceLabel() async throws {
    ...
    try await store.ensureFeedbackInbox()
    try write(
      "I could not tell what to do next.",
      to: store.feedbackInboxURL.appendingPathComponent("feedback.txt"))
    var organization = OrganizationState.seeded(now: Date(timeIntervalSince1970: 100))
    ...
    XCTAssertEqual(result.dutyOccurrence(occurrenceID)?.status, .blocked)
    XCTAssertTrue(
      result.dutyOccurrence(occurrenceID)?.blockingReason?.contains("captured feedback source")
        == true)
    XCTAssertTrue(result.artifacts.isEmpty)
  }
