...
final class LegacyWorkAdaptersTests: XCTestCase {
  func testResearchAssignmentCreatesOneCanonicalOutcomeAndMirrorsDelivery() throws {
    ...
    let assignmentID = try state.createResearchAssignment(
      outcome: "Understand onboarding friction", context: "Use supplied interviews.")
  ...
  func testPreparedFirstMissionIsIdempotentAndEmployeeOwned() throws {
    ...
    XCTAssertEqual(
      Set(first.compactMap { state.employeeOutcome($0)?.assigneeID }), Set(["nia", "theo", "maya"]))
  ...
  func testMigrationLinksLegacyAssignmentWithoutDeletingHistory() throws {
    ...
    legacy.knowledge?.researchAssignments.append(
      ResearchAssignment(
        id: "legacy-research", outcome: "Legacy outcome", context: "Legacy context",
    ...
    let outcomeID = try XCTUnwrap(
      migrated.researchAssignment("legacy-research")?.canonicalOutcomeID)
