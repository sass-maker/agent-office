...
final class WorkforceSupervisionTests: XCTestCase {
  func testContractPlanRequiresApprovalAndDeliveryRequiresAcceptance() async throws {
    ...
    let outcomeID = try state.createEmployeeOutcome(
      employeeID: "maya", outcome: "Prepare an editorial decision", context: "",
      acceptanceCriteria: ["Name one decision"], now: Date(timeIntervalSince1970: 200))
    ...
    try state.approveOutcomePlan(
      outcomeID, note: "Proceed with the bounded plan.", now: Date(timeIntervalSince1970: 310))
    ...
    try state.acceptOutcome(
      outcomeID, note: "This resolves the decision.", now: Date(timeIntervalSince1970: 500))
  ...
  func testContextualReplyResumesOnlyAffectedOutcome() throws {
    ...
    let waitingID = try state.createEmployeeOutcome(
      employeeID: "nia", outcome: "Find evidence", context: "")
    let otherID = try state.createEmployeeOutcome(
      employeeID: "theo", outcome: "Draft note", context: "")
    _ = state.updateEmployeeOutcome(waitingID) {
      $0.status = .waiting
      $0.helpRequest = "Which customer segment?"
    }
    ...
    XCTAssertEqual(
      state.employeeOutcome(waitingID)?.effectiveManagementMessages.last?.actorID, "owner")
  ...
  func testDelegationValidatesEmploymentAndSkillCoverage() throws {
    var state = LocalOrganizationStore.migrated(.seeded())
    let outcomeID = try state.createEmployeeOutcome(
      employeeID: "maya", outcome: "Coordinate a brief", context: "")
    let taskID = "\(outcomeID)-task-1"
    state.tasks.append(
      WorkTask(
        id: taskID, title: "Communicate the result", detail: "Prepare the handoff.", kind: .report,
        status: .ready, assigneeID: "maya", reviewerID: nil, dependencyIDs: [], artifactIDs: [],
        revisionCount: 0, maxRevisions: 0, updatedAt: Date(), accountableEmployeeID: "maya",
        requiredSkillIDs: ["communication"]))
    _ = state.updateEmployeeOutcome(outcomeID) {
      $0.taskIDs = [taskID]
      $0.status = .proposed
    }
    ...
    XCTAssertThrowsError(
      try state.reassignTicket(taskID, to: "theo", reason: "Try another writer.")
    ) {
  ...
  func testQueuePriorityReorderAndBoundedRevisionPreserveDelivery() throws {
    ...
    _ = state.updateEmployeeOutcome(first) { value in
      ...
      value.deliveries = [
        OutcomeDelivery(
          summary: "Original delivery", artifactIDs: [], evidenceBasis: "local",
          limitations: "None", recommendedNextAction: "Review", deliveredByEmployeeID: "theo",
          createdAt: Date())
      ]
    }
    ...
    XCTAssertEqual(
      state.employeeOutcome(first)?.effectiveDeliveries.first?.summary, "Original delivery")
  ...
  private func temporaryDirectory() -> URL {
    FileManager.default.temporaryDirectory.appendingPathComponent(
      "AgentOfficeSupervisionTests-\(UUID().uuidString)", isDirectory: true)
  }
}
