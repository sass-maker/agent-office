...
final class ResearchAssignmentEngineTests: XCTestCase {
  ...
  func testLocalResearchWaitsForPermissionWithoutStartingRunner() async throws {
    ...
    XCTAssertTrue(
      result.researchAssignment(assignmentID)?.blockingReason?.contains("grant") == true)
  ...
  func testLocalResearchRequiresURLBeforeDelivery() async throws {
    ...
    XCTAssertTrue(
      result.researchAssignment(assignmentID)?.blockingReason?.localizedCaseInsensitiveContains(
        "source URL") == true)
    XCTAssertTrue(result.artifacts.isEmpty)
  ...
  func testPermittedLocalResearchDeliversCitedBriefAndCapabilityEvidence() async throws {
    ...
    XCTAssertEqual(
      result.knowledge?.capabilityEvents.suffix(2).map(\.kind), [.started, .succeeded])
    XCTAssertEqual(result.artifacts.filter { $0.taskID == assignmentID }.count, 2)
  ...
  func testUnavailableRuntimeWaitsAndOrdinaryFailureCanRetry() async throws {
    for (runner, expectedStatus) in [
      (
        AnyResearchRunner { _ in throw CodexRunnerError.unavailable },
        ResearchAssignmentStatus.waiting
      ),
  ...
  func testCancellationAndReopenLeaveResearchReadyToResume() async throws {
    ...
    XCTAssertTrue(
      cancelled.researchAssignment(assignmentID)?.blockingReason?.contains("resume") == true)
    ...
    XCTAssertTrue(
      reopened.researchAssignment(assignmentID)?.blockingReason?.contains("resume") == true)
