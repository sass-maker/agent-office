...
- **THEN** available employees walk from their resting areas to the stations or
  meetings required by their current work

#### Scenario: A destination becomes occupied
- **WHEN** an employee's route or destination conflicts with another employee
- **THEN** the scene selects an available nearby route or waiting position
  without overlapping the two characters
...
- **THEN** the two rails support the office with richer employee and work detail
  without competing with the room or resembling HR software
